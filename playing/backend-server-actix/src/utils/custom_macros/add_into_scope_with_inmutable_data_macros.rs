#[macro_export]
macro_rules! add_into_scope_with_inmutable_struct_data {
    
    ($scope:expr,$data:expr, $func:ident) =>{
        add_into_scope!(
            add_struct_data_into_scope!($scope, $data), 
            $func)
    };
    
    ($scope:expr, $data:expr, ($($func:ident),+)) => {
        add_into_scope!(add_struct_data_into_scope!($scope, $data), ($($func),+))
    };
...
mod tests {
    ...
    use crate::{add_into_scope_with_inmutable_struct_data, add_into_scope, add_struct_data_into_scope};
    ...
    fn test_add_into_scope_with_inmutable_data() {
        ...
        let _scope = add_into_scope_with_inmutable_struct_data!(scope, data, path_3);
    }
