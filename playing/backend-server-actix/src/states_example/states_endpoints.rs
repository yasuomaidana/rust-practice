use crate::{
    add_into_scope, add_into_scope_with_inmutable_struct_data,
    add_into_scope_with_mutable_struct_data, add_mutable_struct_data_into_scope,
...
pub async fn counter(counter: web::Data<Counter>) -> impl Responder {
    ...
    *count += 1;
    let message = format!(
        "You have visited this endpoint {count} times",
        count = *count
    );
    HttpResponse::Ok().body(message)
}
